...
仅提交源码、必要测试、公共文档和不含真实值的配置模板。Cookie、账号配置、验证码、二维码、密钥、环境配置、数据库、运行日志、截图、素材和部署备份留在仓库外的私有运行目录；不要使用强制添加绕过忽略规则。

`conf.example.py`、`.env.example`、`.env.*.example`、`accounts.ini.example` 是模板。使用上游历史前端时，从 `sau_frontend` 下的 `.env.*.example` 生成本地同名 `.env.*`；旧小红书入口的 `accounts.ini` 也由模板生成。真实配置受到忽略规则保护，不再直接修改已跟踪的配置文件。

提交前同时检查文件清单和内容差异；`.gitignore` 不会移除已经跟踪的文件，也不会清理旧提交里的内容。Arc Home 的部署记录与服务器备份不放入本公开仓库。
